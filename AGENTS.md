...
- **KV best practices** (namespace must be bound in the EdgeOne console as `luckylab_kv`; code probes `context.env` **and** a global binding):
  ...
  - `list()` is the only key-discovery API and caps at 256 keys per page (`cursor` for more; `ListKey` field is `key`). Deletes of stale keys piggyback on write-path requests (lazy sweep) — there is no background job.
