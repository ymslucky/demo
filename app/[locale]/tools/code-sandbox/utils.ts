/**
 * Code Sandbox —— 核心运行域纯逻辑（无 React / 无副作用，全部可单测）。
 *
 * 关注点分离：本文件只承载“代码运行”域（语言 / 会话 / 控制台 / 运行历史 /
 * 运行响应解析 / 实例档案类型）；实例列表域在 instances.ts，浏览器工作台
 * 域在 browser.ts。代码内容保持 CJK-free（词表走 next-intl）；注释允许中文。
