...
export default async function RateLimiterPost({
  ...
  return (
    <article className="container" style={{ maxWidth: '800px', paddingTop: 'var(--space-xl)', paddingBottom: 'var(--space-2xl)' }}>
      ...
      <div className="blog-content">
        <p style={{ fontSize: 'var(--fs-xl)', fontWeight: 500, color: 'var(--color-text-muted)', marginBottom: '3rem' }}>
          {t("description")}
        </p>
        ...
        <h3>2.1 固定窗口 (Fixed Window)</h3>
        <p>
          最直觉的限流方式。就像汉堡店门口的保安，按小时发号：<span className="blog-inline-code">每小时最多只放 100 个人进去</span>。它的实现极度简单：维护一个计数器，到了下一个时间窗口就把计数器清零。
        </p>

        <CodeBlock 
          title="FixedWindowRateLimiter.java"
  ...
  synchronized boolean tryAcquire() {
    ...
    return false;
        ...
        <div className="blog-callout">
          ...
          结果就是在短短两秒内系统承受了 200 个请求的冲击，限流效果大打折扣！
        </div>

        <h3>2.2 滑动窗口 (Sliding Window)</h3>
        <p>
          为了解决固定窗口的“突刺”问题，滑动窗口应运而生。它不再切分固定的时间块，而是<strong>随时往回看过去一分钟</strong>的请求量。就像保安手里拿着一个精准的秒表，每次有人要进门，他都会翻阅记录本：“过去 60 秒内，进去了多少人？”
        </p>

        <CodeBlock 
          title="SlidingWindowRateLimiter.java"
  ...
  synchronized boolean tryAcquire() {
    ...
    return false;
        ...
        <h3>2.3 漏桶 (Leaky Bucket)</h3>
        <p>
          想象一个底部有小孔的桶，水（请求）以任意速度倒进去，但桶底漏水的速度是<strong>绝对匀速</strong>的。如果倒水的速度太快，桶满了，多余的水就会溢出（请求被拒绝）。
        </p>
        <p>
          漏桶算法的核心目的是<strong>平滑流量</strong>。无论外部流量多么狂暴，系统处理请求的速率始终是一条直线。它非常适合用来保护极其脆弱、一点突发都承受不了的下游系统。
        </p>

        <h3>2.4 令牌桶 (Token Bucket)</h3>
        <p>
        ...
        <p>
          在真实工程中（如 Guava 的 <span className="blog-inline-code">RateLimiter</span>），令牌桶并不是真的启动一个后台线程去定时发令牌（那样太浪费资源了）。而是采用了<strong>惰性计算 (Lazy Computation)</strong>：
        </p>

        <CodeBlock 
          title="TokenBucket.java"
  ...
  synchronized boolean tryAcquire() {
    ...
    return false;
        ...
        <h2>3. 令牌桶的交互演示</h2>
        <p>为了更直观地理解令牌桶是如何“攒令牌”并在瞬间应对突发流量的，你可以尝试点击下方的按钮进行模拟。</p>
