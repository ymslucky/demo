...
- `onRequest*` 处理器保持薄；纯逻辑导出给
  ...
  同一提交中更新该测试。
- **共享判定逻辑以 npm 依赖包复用**：[@lucky/auth-core](../../shared/auth-core/)
  （`file:` 依赖，随 `npm install` 进入 node_modules——边缘函数的模块解析有
  官方 @supabase 示例背书，2026-09 实测生效）。函数入口文件保持薄（编排 +
  响应），纯判定一律 import 自该包并按需 re-export 供测试；**不要**把共享
  逻辑拷贝回函数文件，也**不要**用文本注入/构建脚本做共享（用户否决的 hack）。
...
| 借口 | 现实 |
|---|---|
| "各层各写一份更快" | 同一判定逻辑三份拷贝必然漂移——判定进 @lucky/auth-core，标准 npm import 复用（§2）。 |
